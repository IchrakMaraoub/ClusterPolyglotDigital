...
```bash
mkvirtualenv qualifeed_app -p python3.9
...
Create a database named `qualifeed_app`.

```
createdb qualifeed_app
```
...
```bash
celery -A qualifeed_app worker -l INFO
```

Or with celery beat (for scheduled tasks):

```bash
celery -A qualifeed_app worker -l INFO -B
```
